...
- `tests/gate.sh` — package gate test. Extracts the jq gate from SKILL.md so
  the tested program is exactly the shipped one, and asserts pass/fail
  fixtures (F verdicts, crashed jobs, schema drift, null payloads).
